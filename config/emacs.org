...
(setq dired-omit-files (rx bol ?. (not (any ?.))))
#+end_src
...
(setq-default tab-width 2) ;; TAB (?\t) characters are rendered as being two visual characters wide
(tool-bar-mode -1)         ;; Hide the toolbar with the big buttons
#+end_src
...
#+begin_src elisp
  ...
  (push 'git-commit elpaca-ignored-dependencies)
  (setq elpaca-queue-limit 8)
  (setq elpaca-lock-file (expand-file-name "emacs.lock" (xdg-config-home)))

  (defconst *builtin-package-since*
    '((org . "22")
      (org-tempo . "26.2")
      (project . "25.1")
      (eglot . "29.1")
      (treesit . "29.1")
      (display-line-numbers . "26.1")))
  ...
  (defun is-builtin (sym)
    ...
    (let ((bundled-in))
      (if (and bundled-in (version< bundled-in emacs-version))
          t
...
#+begin_src elisp :noweb yes
      (use-package org
        ...
        :init
        (setq org-directory
          (let ((doc-dir (getenv "XDG_DOCUMENTS_DIR")))
            (if doc-dir
              (f-join doc-dir "note")
              "~/doc/note")))
        :custom
        ...
        (org-agenda-files
          (list
            ...
            (expand-file-name "scratch.org" org-directory)))
        (org-todo-keywords '((sequence "TODO" "DOING" "DONE(!)")))
        (org-todo-keyword-faces '(("DOING" . org-doing)))
        (org-log-into-drawer t)
        (org-export-with-todo-keywords nil) ;; When exporting to LaTeX, don't print the TODO keywords
        :config
        (defface org-doing
          `((t :inherit 'default))
...
#+begin_src elisp
    (use-package org-roam
      ...
      ;;             yeet/goto-notes-weekly org-roam-buffer-toggle)
      :config
...
#+begin_src elisp
  ...
  (use-package ob
    :defer t
  	:ensure nil ;; shipped with org-mode
    :after org
    :custom
...
#+begin_src elisp
  (use-package org-latex
    ...
    (setq org-startup-with-latex-preview t
          ...
          org-preview-latex-default-process 'tectonic-svg
    ...
    (setq org-preview-latex-process-alist
      (map-insert org-preview-latex-process-alist 'tectonic-svg
...
(use-package org-noter :if (not (eq system-type 'windows-nt)))
#+end_src
...
For some reason the global mode can't be enable from a hook.
...
(use-package treesit
  :after dash
  :ensure `,(maybe-recipe 'treesit)
  :config
  (setq treesit-language-source-alist
    '((c     . ("https://github.com/tree-sitter/tree-sitter-c"))
      (cpp   . ("https://github.com/tree-sitter/tree-sitter-cpp"))
      (bison . ("https://gitlab.com/btuin2/tree-sitter-bison"))))
  ;; FIXME: check if `git' and `cc' are available before attempting to install parsers
  (dolist (source treesit-language-source-alist)
    (unless (treesit-ready-p (car source))
      (treesit-install-language-grammar (car source))))
  (setq auto-mode-alist
    (--> auto-mode-alist
      (map-insert it "\\.c\\'"   #'c-ts-mode)
      (map-insert it "\\.cpp\\'" #'c++-ts-mode)
      (map-insert it "\\.y\\'"   #'bison-ts-mode))))
...
(use-package java-mode
  :ensure nil
  :hook (java-mode . yeet/disable-tabs))
...
(use-package nim-mode
  ...
  (add-to-list 'eglot-server-programs
    (cons 'nim-mode
      (eglot-alternatives '(("nimlangserver"))))))
...
(use-package uiua-mode
  :mode "\\.ua\\'"
  :ensure (uiua-mode :protocol https
     :inherit t
     :depth 1
     :host github
...
(use-package nael
  ...
  (nael
    ...
    :repo "mekeor/nael"
    :files (:defaults "nael/*"))
  :hook ((nael-mode . yeet/disable-tabs)))
#+end_src
...
(use-package scheme-mode
  :ensure nil
  :hook ((scheme-mode . yeet/disable-tabs)
         (scheme-mode . yeet/lisp-prettify-symbols)))
#+end_src
...
(use-package clang-format
  :hook ((c-mode c++-mode c-ts-mode c++-ts-mode) . (lambda () (add-hook 'before-save-hook 'clang-format-buffer nil t)))
  :config
...
#+RESULTS: full-c-cpp
: [nil 27137 12661 854681 nil elpaca-process-queues nil nil 38000 nil]
...
(use-package fence-edit
  :ensure (fence-edit :type git :host github :repo "aaronbieber/fence-edit.el")
...
#+begin_src elisp
(use-package j-mode
  :ensure (j-mode :type git :host github :repo "LdBeth/j-mode")
  :config (setq j-console-cmd "jconsole"))

(use-package ob-J
  :ensure nil ;; comes from `'org-contrib'
...
#+begin_src elisp
  ...
  (use-package theme-changer
    :after battery
    :config
    ;; FIXME: This is really slow, find some way to cache this
    ;; (let ((map (yeet/get-geolocation)))
    ;;   (setq calendar-latitude  (map-elt map "lat")
    ;;         calendar-longitude (map-elt map "lon")))
    (setq calendar-latitude  -24.72274
          calendar-longitude -53.74082)
    (change-theme 'battery-light 'battery-dark))
#+end_src

** frame custom title and smooth resizing

#+name: frames
#+begin_src elisp
;; Terminal mode does not support changing the title, even though terminals do have that capability
(setq frame-title-format
      '(buffer-line-name "Emacs %b (%f)" "Emacs %b"))

;; When this is nil, Emacs will attempt to resize in increments of the size of
;; the character boxes, instead of pixel by pixel.
...
#+name: visual-bell
#+begin_src elisp
  ;; When visible-bell is non-nil, Emacs will flash the whole frame when ringing. Let's just flash the modeline for 0.1 seconds, instead.
...
Highlights trailing whitespace (tabs and spaces) with the =whitespace-trailing= face (red background).
...
(setopt visible-cursor nil) ;; Same thing, but just on the terminal client
#+end_src
